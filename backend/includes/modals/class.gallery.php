...
class Gallery extends DatabaseObject
{
    ...
    static function getGalleryList($total, $offset = 0)
    {
        global $db;
        return self::find_by_sql("SELECT * FROM " . self::$table_name . " WHERE status=1 ORDER BY sortorder ASC LIMIT {$total} OFFSET {$offset}");
    }

    public static function getParentgallery($pid = '1')
    {
        global $db;
        $sql = "SELECT id, slug, title FROM " . self::$table_name . " WHERE status='1' AND type='$pid' ORDER BY sortorder DESC ";
        return self::find_by_sql($sql);
    }
